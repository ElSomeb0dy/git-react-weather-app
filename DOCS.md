...
GROQ_API_KEY=your_groq_key_here   # optional — AI insight falls back to local summary without it
```
...
├── services/
│   ├── aiInsight.ts             # Groq AI weather insight with AsyncStorage cache
│   ├── cities.ts                # Supabase CRUD for saved cities (city, lat, lon)
...
├── hooks/
│   ├── useLocalTime.ts          # Local time string from timezone offset, ticks every 10s
