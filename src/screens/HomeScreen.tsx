...
export default function HomeScreen({ navigation }: Props) {
  ...
  const { activeTheme, locationCondition, locationIsNight, topCondition, topIsNight } =
    useMemo(() => {
      ...
      const nightFor = (w?: CurrentWeather | null) =>
        isNightTime(w?.sunrise ?? null, w?.sunset ?? null);
  ...
  return (
    <View
      style={[
        styles.container,
        { backgroundColor: activeTheme.background, paddingTop: insets.top + 16 },
      ]}
    >
      ...
      {locationWeather && (
        <View style={[styles.locationCard, { backgroundColor: activeTheme.card }]}>
          ...
          {locationLocalTime ? (
            <Text style={[styles.locationTime, { color: activeTheme.subtleText }]}>
              {locationLocalTime}
            </Text>
          ) : null}
          ...
          {locationSlots.length > 0 && (
            ...
            >
              {locationSlots.map((slot) => (
                <View
                  key={slot.time}
                  style={[styles.slotCard, { backgroundColor: activeTheme.background }]}
                >
