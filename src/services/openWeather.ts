...
function parseWeatherData(data: any): CurrentWeather {
  ...
  return {
    ...
    condition,
    description: (data.weather?.[0]?.description ?? "").replace(/\b\w/g, (ch: string) =>
      ch.toUpperCase()
    ),
...
function parseForecast(data: any): ForecastDay[] {
  ...
  const days = [...byDay.entries()]
    .filter(([d]) => d !== today && byDay.get(d)!.length > 0)
    .slice(0, 5);
  return days.map(([day, slots]) => {
    ...
    const description = (mid.weather?.[0]?.description ?? "").replace(/\b\w/g, (ch: string) =>
      ch.toUpperCase()
    );
    return {
