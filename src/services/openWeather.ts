...
function parseForecast(data: any): ForecastDay[] {
    ...
    return days.map(([day, slots]) => {
        ...
        const description = (mid.weather?.[0]?.description ?? "").replace(/\b\w/g, (ch: string) => ch.toUpperCase());
        return {
            date: new Date(day).getTime(),
            minTempC: minC, maxTempC: maxC,
            minTempF: round1dp(cToF(minC)), maxTempF: round1dp(cToF(maxC)),
